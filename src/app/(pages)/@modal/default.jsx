...
export default function Preview() {
  ...
  return (
    <div className={isModal ? "hidden" : "fixed shadow-dim shadow-sm bg-slate-400 dark:bg-slate-600 dark:bg-opacity-30 bg-opacity-30 z-40 w-full h-screen left-0 top-0 justify-center items-center"}>
      {specificGood.map((stock, index) => (
        <div key={index} className="rounded-3xl w-4/6 m-auto my-20 h-auto gap-4 flex flex-row dark:bg-slate-800 bg-white p-4 box-border">
          <IoClose onClick={() => setIsModal(!isModal)} className="absolute z-50 text-slate-400 dark:text-slate-300 rounded-full text-4xl " />
          ...
          <div className="w-2/5 max-md:w-full h-auto gap-2 flex flex-col">
              <h2 className="font-bold text-lg text-slate-400 dark:text-gray-200">{stock.title}</h2>
              <div className="flex gap-2 flex-col">
                <div className="font-normal text-xs text-slate-600 dark:text-gray-400 inline-flex gap-1" ><Rating />
                ...
                <div className="font-normal text-xs text-slate-600 dark:text-gray-400 flex gap-4">
                  ...
                  <p className=" text-center flex flex-col ">
                    <span className=" text-[24px]  text-red-600 dark:text-slate-500 font-semibold text-center" onClick={() => setLiked(!liked)}>
                      {liked ? <IoHeart /> : <IoHeartOutline className="text-slate-400 dark:text-slate-300" />}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <form>
                <div className="w-auto h-auto box-border text-lg font-semibold">
                  <h3 className=" text-slate-600 dark:text-gray-200">Color: blue</h3>
                ...
                <div className=" w-auto h-auto box-border text-lg font-semibold">
                  ...
                  <div className="gap-2 flex h-8 flex-row ">
                    ...
                    <label htmlFor="42" className="w-8 h-full bg-slate-200 dark:bg-slate-400 p-2 text-sm flex justify-center items-center peer-checked/42:border-primary rounded-xl text-slate-400 dark:text-slate-300 border-2"><p>42</p></label>
                    <input checked type="radio" className="sr-only peer/43" name="color_blue" id="43" />
                    <label htmlFor="43" className="w-8 h-full bg-slate-200 dark:bg-slate-400 p-2 text-sm flex justify-center items-center peer-checked/43:border-primary rounded-xl text-slate-400 dark:text-slate-300 border-2"><p>42</p></label>
                  </div>
                </div>
                {/* <div className=" w-auto h-auto box-border text-lg font-semibold">
            ...
            <div className=" w-auto h-auto box-border text-lg font-semibold">
              ...
              <div className="gap-2 flex h-8 flex-row ">
                ...
                <div className=" w-auto h-auto box-border text-lg font-semibold">
                  <h3 className=" text-slate-600 dark:text-gray-200">Quantity:</h3>
                  <div className=" p-1 w-24 h-8 gap-2 flex text-slate-600 items-center text-1xl outline-0 border-0 bg-slate-200 dark:bg-slate-400 rounded-full cursor-pointer ">
                    <button onClick={(e) => {e.currentTarget.disabled = true;  setStockCount(prev=>prev-1); stockCount == 0 && removeEventListener("click")}} className=" text-2xl text-white bg-primary dark:bg-secondary rounded-full" >
                      <IoRemove />
                    </button>
                    <p className=" w-full border-0 outline-0 text-center bg-transparent">{stockCount}
                    </p>
                    <button onClick={() => setStockCount(prev=>prev+1)} className=" text-2xl text-white dark:text-slate-800 bg-primary dark:bg-secondary rounded-full" >
                      <IoAdd />
                    </button>
                  </div>
                </div>
