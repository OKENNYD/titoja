  ...
  return (
    <aside className="p-4 max-md:hidden h-fit w-1/4 rounded-2xl bg-white dark:bg-dark cursor-pointer sticky ">
      ...
      <ul className=" h-auto scroll-none overflow-y-scroll">
        ...
        <li>
          <p className="text-base font-semibold text-dim dark:text-gray-200">
            Size
          </p>
          <ul className="ms-2 accent-secondary">
            {sizes.map((_s, i) => (
              <li
                key={i}
                className="flex gap-1 text-base dark:text-gray-400 font-medium "
              >
        ...
        <li>
          ...
          <ul className="ms-2 accent-secondary">
            {sizes.map((_s, i) => (
              <li
                key={i}
                className="flex gap-1 text-sm dark:text-gray-400 font-medium "
              >
                <input type="checkbox" name="category" id={_s.range} />
                <label htmlFor={_s.range}>{_s.range}</label>
              </li>
        ...
        <li>
          <p className="text-base font-semibold text-slate-600 dark:text-gray-200">
            Materials
          </p>
          <ul className="ms-2 accent-secondary">
            {materials.map((_m, i) => (
